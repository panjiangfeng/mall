...
public class GoodsController {
    ...
    public String allgoodspagequery(@RequestParam("pageNum") int pageNum,
        ...
        for (int i = 0; i < goodslist.size(); i++) {
            ...
            wrappergoods.eq("goodsid", goodslist.get(i).getGoodsid());
...
//       购买成功库存后续操作,库存减少,优惠券减少，销量加一等等
    ...
    public String buyMultiple(@RequestBody Map<String, Object> params) {
        if (params.containsKey("userid") == false||params.containsKey("goods")==false) {
            return "failed";
        }
        Long userid = Long.parseLong((String) params.get("userid")) ;

        List<Map> buyGoods = (List<Map>) params.get("goods");
        /*
         * 拿到user对象     userinfo对象     goods对象  goodsimg对象  取出需要的属性存入order对象中
         * */
        for (int i = 0; i < buyGoods.size(); i++) {
            if(buyGoods.get(i).containsKey("id")==false||buyGoods.get(i).containsKey("choosesize")==false){
                return "failed";
            }
            Long goodsid = Long.parseLong(String.valueOf(buyGoods.get(i).get("id")));
            String size = (String) buyGoods.get(i).get("choosesize");
            QueryWrapper wrapperuser = new QueryWrapper();
            wrapperuser.eq("id",userid);
            User user = userMapper.selectOne(wrapperuser);
...
//       购买成功库存后续操作,库存减少,优惠券减少，销量加一等等
    ...
    @RequestMapping(value = "/goodsimg/{id}", method = RequestMethod.GET, produces = "image/png")
    public byte[] getGoodsImg(@PathVariable String id) throws IOException {
        String currentPath = System.getProperty("user.dir");
        File path = new File(currentPath + "/goodsimg/" + id); //服务器地址
    ...
    public String uploadgoodsimg(@RequestParam("file") MultipartFile file,
        ...
        //上传文件
        String currentPath = System.getProperty("user.dir");
        File path = new File(currentPath + "/goodsimg"); //服务器地址
        if (!path.exists()) {
            path.mkdir();
        }
